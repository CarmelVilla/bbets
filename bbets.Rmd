...
We are trying to find out how much brownlow will win over class size and pay 
offs. The data was found on Brownlow profile.
...
On average Brownlow loss is 2 to 1.
```{r}
numInClass = 35
odds = 2
...
```
The average win per class is 7.47 for classroom size of 35. 
...
print(paste('Average win/loss per class:',round(bet1,2)))
```
The average win per class would be 8.26 cents per class size of 35 and odds of 2.
 

  
<!--Explain and display your method of coming up with a 95% confidence interval of  professor Brownlow's winnings (or losings) !-->
run a bunch of classes and look at histogram.
...
hist(money)
```
The answer would be 8 dollars per class of 35. As graph displays.
...
hist(money)
```
Based on the graph my answer should be around 4.5 with this pay out it will make it fair.
...
With a class of 28 and odds of 2 this makes it a zero cash pay out.As the graph also displays.
...
to conclude Proffesor Brownlow with a class of 35 and odds of 4.5 this makes the cash pay out on average zero profit. this will continue his schemed balance at zero. 
